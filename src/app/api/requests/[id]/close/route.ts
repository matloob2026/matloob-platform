import { NextResponse } from "next/server";
import { auth } from "@/auth/auth";
import { requestService, RequestServiceError, requestServiceErrorStatus } from "@/services/request.service";
import type { ApiError } from "@/types/domain";

export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const session = await auth();
  if (!session?.user?.id) {
    const error: ApiError = { code: "UNAUTHENTICATED", message: "You must be signed in to close a request." };
    return NextResponse.json({ error }, { status: 401 });
  }

  try {
    await requestService.close(id, session.user.id);
    return NextResponse.json({ data: { closed: true } }, { status: 200 });
  } catch (err) {
    if (err instanceof RequestServiceError) {
      const error: ApiError = { code: err.code, message: err.message };
      return NextResponse.json({ error }, { status: requestServiceErrorStatus(err.code) });
    }
    console.error("POST /api/requests/[id]/close", err);
    const error: ApiError = { code: "UNKNOWN_ERROR", message: "Could not close the request." };
    return NextResponse.json({ error }, { status: 500 });
  }
}
